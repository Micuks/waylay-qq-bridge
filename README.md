...
`13000` 端口除了承担 Bridge WebSocket 之外，还内置一个基于 Waylay 设计系统的浏览器控制台。无需额外服务、无需打包构建，直接由 Bridge 进程提供静态资源。

| 路径 | 说明 |
|------|------|
| `GET /` | 落地页：品牌 / 实时状态 / 架构 / 延迟对比表（浏览器请求 → HTML，其它请求 → JSON） |
| `GET /docs` | 文档站（侧边栏 + 内容区，覆盖 quickstart / install / 协议 / API / 错误码 / 对比） |
...
支持浅色 (Parchment) / 深色 (Obsidian) 双主题，遵循系统偏好；标头按钮可手动切换，并写入 `localStorage`。`?theme=dark` / `?theme=light` 也可强制。
